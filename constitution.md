...
- R02a — **Directory names in `snake_case` (non-negotiable).** Every directory under `lib/` and `test/` — module folders included — separates words with `_`: `driver_search`, `driver_service_areas`, `design_system`, `value_objects` — never the run-together `driversearch` / `driverserviceareas`. Single-word folders are unchanged (`auth`, `profile`, `data`, `domain`, `presentation`). This is the Dart style guide rule (`lowercase_with_underscores` for both files and directories), already applied to every `.dart` file name here; folders were the gap. Only the path segment is affected — Dart identifiers keep their normal casing (`DriverSearchCubit`, `driverSearchState`). `flutter analyze` does not inspect directory names, so this is a review item. Note this rule is Dart-specific: the Java repo keeps the JLS convention of unseparated lowercase packages (`rolepermission`), which is the correct form *there*.
...
- R40a — **No comments in source code (non-negotiable).** Source files (`lib/**`, `test/**`, and any hand-written `.dart`) must contain no comments — not doc comments (`///`), line comments (`//`), or block comments (`/* */`). Names and small, single-purpose functions (R31–R39) must carry the intent instead. Comments are allowed **only** in important configuration files where a value's meaning or "why" is not self-evident from the code — e.g. `.env.example`, service/build/CI configuration, and design-token or environment definitions. This rule does not apply to generated files (`*.g.dart`, `*.freezed.dart`, `l10n/app_localizations*`), which are regenerated, nor to tool/analyzer directives that are not prose (e.g. `// ignore:`, `// coverage:ignore`). If a comment feels necessary in source code, treat it as a signal to rename or restructure (R34); if code genuinely cannot be made self-explanatory — e.g. an intentionally empty `catch` — restructure to avoid it (use `.catchError`, an early return, or a named helper) rather than adding a comment.
