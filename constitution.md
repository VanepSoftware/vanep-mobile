...
- R40a — **Comments in source are the exception.** Default: names and small functions (R31–R40) carry intent — do not narrate code, restate signatures, or add section banners. Source files (`lib/**`, `test/**`, and any hand-written `.dart`) **may** contain a comment only when it is **critically necessary**, in one of these cases: (1) a business or protocol rule that cannot be made obvious by naming — e.g. an invariant the type system cannot express; (2) a `TODO` / `FIXME` that records debt, the intended fix, and other call sites that share the same problem; (3) a workaround for an external bug or constraint whose "why" is not in the code. Configuration files may still explain non-obvious values (`.env.example`, CI, design tokens). This rule does not apply to generated files (`*.g.dart`, `*.freezed.dart`, `l10n/app_localizations*`) nor to analyzer directives (`// ignore:`, `// coverage:ignore`). If the comment would only explain structure or the "what", rename or split (R34) instead.
